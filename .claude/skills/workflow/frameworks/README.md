...
`/workflow <task> --framework <name>` loads `frameworks/<name>.md` (case-sensitive file name, e.g. `--framework AIDLC` → `AIDLC.md`). With no flag, the default is **AIDLC**. If the named file doesn't exist, the skill lists this directory (excluding `README.md` and `_TEMPLATE.md`) and asks which framework to use.
