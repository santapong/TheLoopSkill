...
When the hypotheses are **independent and do not share a cheap discriminating test** — so testing them serially is slow and no single observation splits the set — run them in parallel. This is the `../SKILL.md` §8 orchestration path, built on the `../../workflow` skill's `templates/parallel.workflow.js`:
...
| `../../workflow/references/harness-policy.md` | H2 earned barrier, H3 structured output, H4 adversarial/diverse verify, H5 null handling — cited throughout §7 |
</content>
</invoke>
