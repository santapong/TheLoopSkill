...
Number ADRs sequentially, keep a `Status` (Proposed · Accepted · Deprecated · Superseded), and store them in `docs/adr/NNNN-slug.md`. For the full blank template, section-by-section guidance, and the point that the rejected **Alternatives Considered** are what make it an ADR rather than a changelog entry, use **`../../designing-systems/templates/adr-template.md`** — do not duplicate that structure here, fill it in there.
