...
When a stage fans out across many items, the governing constraint is no longer "which model reasons best" but **budget** — the token target is a hard ceiling, and once it is spent further `agent()` calls throw (harness policy H6, see `../../workflow/references/harness-policy.md`). Protect that ceiling by dropping the per-item tier one step:
