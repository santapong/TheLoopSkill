...
- **Never silently drop a plausible Critical.** A genuine Critical at 0.6 confidence deserves the adversarial verifier (`../../workflow/references/harness-policy.md` H4), not the trash. Route it there; if it clears, report it; if it dies, log why so a human can second-guess the drop.
