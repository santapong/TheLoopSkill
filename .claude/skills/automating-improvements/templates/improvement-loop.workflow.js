...
do {
  ...
  const handled = await pipeline(
    ...
    (it) =>
      agent(
        `Triage this ${it.kind} for ${REPO.owner}/${REPO.name} and decide the approach + which sibling skill owns it (diagnosing-bugs, designing-systems, writing-tests, finding-frameworks). Item: ${it.title} — ${it.detail || ''} (${it.ref || ''}). Set worthDoing=false to skip.`,
    ...
    (prev) => {
      ...
      return agent(
        `Adversarially review this change and write its impact/risk memo (reviewing-code + auditing-changes). Set safeToPropose=false if it is unsafe, unclear, or unverified.\nChange: ${prev.act.summary}\nFiles: ${(prev.act.filesChanged || []).join(', ')}`,
