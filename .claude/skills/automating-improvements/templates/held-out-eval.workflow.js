...
const measured = await pipeline(
  ...
  (prev) => {
    ...
    return agent(
      `Judge this candidate with the project's CURRENT verify configuration — the exact rubric reviewing-code + auditing-changes apply in the improvement loop, including any current credit-ledger context. Do not use a stricter or looser bar than the live loop. Set safeToPropose the way the live loop would.\nChange: ${prev.act.summary}\nFiles: ${(prev.act.filesChanged || []).join(', ')}`,
