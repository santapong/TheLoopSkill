...
The autonomous improvement loop in full. It reuses the `workflow` skill's loop and harness policies — this file is how those rules apply to a self-improving project, not a new set of rules.
...
Not all feedback is equal. Within a round, order the batch by impact × urgency: red CI and reported bugs first (they block others), then requested changes on open PRs, then issues, then proactive tech-debt and research ideas. Use `orchestrating-projects` to turn a large batch into an ordered task DAG and to pick the model/effort per item (cheap triage on a small model, careful fixes and verification on a strong one).

## Act as an AIDLC Construction pass

Each accepted item runs design → implement → test on a `claude/` branch, routed to the owning skill (`diagnosing-bugs`, `designing-systems`, `writing-tests`, `writing-docs`), with `finding-frameworks` as the pre-build "don't reinvent it" gate. Keep changes small and single-purpose — one item, one focused PR.

## Verify before proposing

Adversarially self-review with `reviewing-code` and package the impact with `auditing-changes`. A change that fails review or whose risk memo flags it as unsafe/unclear is **not proposed** — it's dropped or turned into an issue that explains the blocker. Better to propose nothing than to propose noise; the loop's credibility is its signal-to-noise.
