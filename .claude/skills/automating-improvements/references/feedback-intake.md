...
When 1 and 2 return nothing fresh, switch to discovery with `researching-topics`:
...
- `reviewing-code` — security/quality issues worth fixing.
- `auditing-changes` — risk/coverage gaps in recent changes.
- `finding-frameworks` — places the project reinvents something a maintained library already does.
