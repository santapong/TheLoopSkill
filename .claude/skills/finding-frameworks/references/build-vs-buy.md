...
State the pick, the rationale against the must-haves, the **strongest counter-argument named**, and the runner-up. When the choice is architecturally significant or hard to reverse, capture it as an ADR (see the `designing-systems` skill's `templates/adr-template.md`) so the reasoning survives.
