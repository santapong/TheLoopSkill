...
This is the parallel-sweep → pipeline → synthesis pattern from the **`workflow`** skill (see its `templates/parallel.workflow.js`, `templates/pipeline.workflow.js`, and harness policy H2/H4). Invoke the `workflow` skill to author and execute the run; the research template pre-wires the search angles, claim schema, and adversarial verification. For a question small enough to answer from two or three sources, skip the workflow and research directly.
