...
This is the case-design and quality half of the skill. For where tests sit in the review/verify loop, see `../SKILL.md`; for scaling a large suite-authoring or backfill job across agents, the barrier and verification discipline is the same one in `../../workflow/references/harness-policy.md` (H2 earned barrier, H4 adversarial verify).
...
- **Private methods directly.** Test them through the public method that calls them; a private method with no public path is dead code (`../../reviewing-code/references/quality-checks.md` → *Dead code*).
